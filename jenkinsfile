...
pipeline {
    ...
    environment {
        DOCKER_USERNAME = "anta7" // username docker
    ...
    // Les étapes du pipelinegit
    stages {
        // Étape 1 : Récupération du code source depuis GitHub
        stage("Checkout") {
            steps {
                git branch: 'master', url: 'https://github.com/Fablab-Quenn/TP5_A.git'
            }
        }
        ...
        stage("Build Docker Image") {
            steps {
                script {
                    ...
                    echo "Image Docker creee : $DOCKER_IMAGE"
                }
            }
        }
